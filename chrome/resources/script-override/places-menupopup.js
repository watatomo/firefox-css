...
// leaking to window scope.
{
    ...
    `;
            ...
            this._overFolder = {
                ...
                clear: function OF__clear() {
                    if (this._folder.elt && this._folder.elt.lastElementChild) {
                        if (
                            !this._folder.elt.lastElementChild.hasAttribute(
                                "dragover"
                            )
                        ) {
        ...
        _getDropPoint(aEvent) {
            ...
            let dropPoint = { ip: null, folderElt: null };
            ...
            if (
                ...
                if (eventY - eltY < eltHeight * 0.2) {
                    ...
                    dropPoint.ip = new PlacesInsertionPoint({
                        parentId: PlacesUtils.getConcreteItemId(
                            elt._placesNode
                        ),
                        parentGuid: PlacesUtils.getConcreteItemGuid(
                            elt._placesNode
                        ),
                        tagName
                    });
        ...
        on_dragover(event) {
            ...
            if (event.originalTarget == this.scrollBox._scrollButtonUp) {
                ...
                event.originalTarget == this.scrollBox._scrollButtonDown
            ) {
                scrollDir = 1;
            }
            if (scrollDir != 0) {
            ...
            if (scrollDir == 0) {
                ...
                while (
                    elt &&
                    event.screenY >
                        elt.screenY + elt.getBoundingClientRect().height / 2
                ) {
            ...
            newMarginTop +=
                scrollRect.y -
                this._indicatorBar.parentNode.getBoundingClientRect().y;
    ...
    class MozPlacesPopupArrow extends MozPlacesPopup {
        ...
        _setSideAttribute(event) {
            ...
            if (
                position.indexOf("start_") == 0 ||
                position.indexOf("end_") == 0
            ) {
