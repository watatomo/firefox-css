...
class ScriptData {
    constructor(leafName, headerText) {
        ...
        this.inbackground =
            this.isESM || /\/\/ @backgroundmodule\b/.test(headerText);
...
class ScriptInfo {
    ...
    static fromString(aName, aString) {
        const headerText = aString.match(
            /^\/\/ ==UserScript==\s*\n(?:.*\n)*?\/\/ ==\/UserScript==\s*\n/m
        );
...
const utils = {
    ...
    get brandName() {
        return AppConstants.MOZ_APP_DISPLAYNAME_DO_NOT_USE;
    },
    ...
    writeFile: async function (path, content, options = {}) {
        ...
        const disallowUnsafeWrites = !yPref.get(
    ...
    get chromeDir() {
        return {
            get files() {
                ...
                return dir.directoryEntries.QueryInterface(
    ...
    getScriptData: () => {
        ...
        return _ucjs.scripts.map((script) =>
    ...
    get windows() {
        return {
            get: function (onlyBrowsers = true) {
                let windowType = APP_VARIANT.FIREFOX
                    ? "navigator:browser"
                    : "mail:3pane";
                let windows = Services.wm.getEnumerator(
                    onlyBrowsers ? windowType : null
                );
                let wins = [];
    ...
    showNotification: async function (description) {
        if (APP_VARIANT.THUNDERBIRD) {
            console.warn(
                '_ucUtils.showNotification is not supported on Thunderbird\nNotification label was: "' +
                    description.label +
                    '"'
            );
            return;
    ...
    parseStringAsScriptInfo: (aName, aString) =>
        ScriptInfo.fromString(aName, aString)
};
...
class UserChrome_js {
    ...
    init() {
        ...
        // gBrowserHack setup
        const gBrowserHackRequired = yPref.get(
    ...
    `);
      ...
      `)
        ...
        document.l10n
            .formatValues(["restart-button-label", "clear-startup-cache-label"])
            .then((values) => {
                ...
                document
                    .getElementById("userScriptsClearCache")
                    .setAttribute(
                        "label",
