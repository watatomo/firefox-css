export class FileSystem {
    ...
    static getFileURIForFile(aEntry, type) {
        ...
        throw ResultError.fromKind(this.ERROR_KIND_INVALID_ARGUMENT, {
            expected: "FileSystem.RESULT_FILE | FileSystem.RESULT_DIRECTORY",
        });
    }
    ...
    static resolveChromePath(str) {
        ...
        return parts
            .slice(parts.indexOf("chrome") + 1, parts.length - 1)
            .join("/");
    }
    ...
    static {
        this.SCRIPT_DIR = this.resolveChromePath(
            "chrome://userscripts/content/"
        );
        this.RESOURCE_DIR = this.resolveChromePath(
            "chrome://userchrome/content/"
        );
        this.BASE_FILEURI = this.getFileURIForFile(
            Services.dirsvc.get("UChrm", Ci.nsIFile),
            this.RESULT_DIRECTORY
        );
    }

    static #getEntry(aFilename, baseDirectory) {
        if (typeof aFilename !== "string") {
            return FileSystemResult.fromErrorKind(
                FileSystem.ERROR_KIND_INVALID_ARGUMENT,
                "String"
            );
        }
        const filename = aFilename.replace("\\", "/");
        let pathParts = (
            (filename.startsWith("..") ? "" : baseDirectory) +
            "/" +
            filename
        )
            .split("/")
        ...
        if (!entry.exists()) {
            return FileSystemResult.fromErrorKind(
                FileSystem.ERROR_KIND_NOT_EXIST
            );
        }
    ...
    static getEntry(aFilename, options = {}) {
        return this.#getEntry(
            aFilename,
            options.baseDirectory || this.RESOURCE_DIR
        );
    }
    ...
    static readFileSync(aFile, options = {}) {
        if (typeof aFile === "string") {
            ...
            !(aFile instanceof Ci.nsIFile && aFile.exists() && aFile.isFile())
        ) {
            return FileSystemResult.fromErrorKind(this.ERROR_KIND_NOT_FILE);
        }
        ...
        try {
            ...
            cvstream.close();
            stream.close();
            return FileSystemResult.fromError(
                this.ERROR_KIND_NOT_READABLE,
                { cause: e },
        ...
        let rv = {
            content: "",
            path: this.getFileURIForFile(aFile, this.RESULT_FILE),
        };
        let data = {};
    ...
    static async readFile(aPath) {
        ...
        try {
            ...
            console.error(ex);
            return FileSystemResult.fromError(this.ERROR_KIND_NOT_READABLE, {
                cause: ex,
            });
        }
    }
    static async readJSON(path) {
        try {
            ...
            return result.isError() ? null : JSON.parse(result.content());
        } catch (ex) {
            console.error(ex);
        }
        return null;
    ...
    static async writeFile(path, content, options = {}) {
        ...
        const disallowUnsafeWrites = !Services.prefs.getBoolPref(
            "userChromeJS.allowUnsafeWrites"
        );
...
class FileSystemResult {
    ...
    content(replaceNewlines) {
        ...
        throw ResultError.fromKind(
            FileSystem.ERROR_KIND_NOT_CONTENT,
    ...
    read() {
        if (!this.isFile()) {
            return Promise.reject(
                ResultError.fromKind(FileSystem.ERROR_KIND_NOT_FILE)
            );
        }
    ...
    entries() {
        ...
        let enumerator = this.#result.directoryEntries.QueryInterface(
    ...
    showInFileManager() {
        try {
            ...
            console.error(
                "Could not open file manager for: " + this.#result.leafName
            );
        }
        return false;
    }
    ...
    static fromError(aKind, aErrorDescription) {
        return new FileSystemResult(
            new ResultError(aKind, aErrorDescription),
            FileSystem.RESULT_ERROR
        );
    }
    static fromErrorKind(aKind) {
        return new FileSystemResult(
            ResultError.fromKind(aKind),
            FileSystem.RESULT_ERROR
        );
    }
    ...
    static fromNsIFile(entry) {
        ...
        return FileSystemResult.fromError(
            ResultError.fromKind(FileSystem.ERROR_KIND_UNKNOWN_RESULT)
        );
    }
}
