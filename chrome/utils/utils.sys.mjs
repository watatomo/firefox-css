...
function updateStyleSheet(name, type) {
    ...
    // "probably" fine.
    let entryFilePath = `file:///${fsResult
        .entry()
        .path.replaceAll("\\", "/")}`;
...
export class _ucUtils {
    static get appVariant() {
        return loaderModuleLink.variant.THUNDERBIRD ? "Thunderbird" : "Firefox";
    }
    ...
    static createWidget(desc) {
        if (!desc || !desc.id) {
            throw new Error(
                "custom widget description is missing 'id' property"
            );
        }
        if (!(desc.type === "toolbarbutton" || desc.type === "toolbaritem")) {
            throw new Error(
                `custom widget has unsupported type: '${desc.type}'`
            );
        }
        ...
        loaderModuleLink.sharedGlobal.widgetCallbacks.set(
            desc.id,
            desc.callback
        );
    ...
    static getScriptData(aFilter) {
        ...
        if (
            aFilter &&
            !(filterType === "string" || filterType === "function")
        ) {
            throw (
                "getScriptData() called with invalid filter type: " + filterType
            );
        }
        ...
        if (filterType === "function") {
            return _ucScripts
                .filter(aFilter)
                .map((script) =>
                    script.getInfo(!disabledScripts.includes(script.filename))
                );
        }
        return _ucScripts.map((script) =>
            script.getInfo(!disabledScripts.includes(script.filename))
        );
    }
    ...
    static parseStringAsScriptInfo(aName, aString) {
        return ScriptInfo.fromString(
            aName,
            FS.StringContent({ content: aString })
        );
    }
    static prefs = {
            ...
            _ucUtils.windows.forEach((doc, win) => {
                ...
                let keyset =
                    ...
                    doc.body.appendChild(
                        _ucUtils.createElement(doc, "keyset", { id: "ucKeys" })
                    );
    ...
    static get windows() {
        return {
            get: function (onlyBrowsers = true) {
                let windowType = loaderModuleLink.variant.FIREFOX
                    ? "navigator:browser"
                    : "mail:3pane";
