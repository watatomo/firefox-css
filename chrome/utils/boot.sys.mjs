...
class ScriptData {
    ...
    constructor(leafName, headerText, noExec) {
        const hasLongDescription = /^\/\/\ @long-description/im.test(
            headerText
        );
        ...
        this.homepageURL = headerText.match(
            /\/\/ @homepageURL\s+(.+)\s*$/im
        )?.[1];
        this.downloadURL = headerText.match(
            /\/\/ @downloadURL\s+(.+)\s*$/im
        )?.[1];
        this.updateURL = headerText.match(/\/\/ @updateURL\s+(.+)\s*$/im)?.[1];
        this.optionsURL = headerText.match(
            /\/\/ @optionsURL\s+(.+)\s*$/im
        )?.[1];
        ...
        let exclude = rex.exclude.length
            ? `(?!${rex.exclude.join("$|")}$)`
            : "";
        ...
        if (this.inbackground) {
            ...
            let loadOrder = headerText.match(
                /\/\/ @loadOrder\s+(\d+)\s*$/im
            )?.[1];
    ...
    preCompileMJS() {
        ...
        this.#preCompiling = new Promise((resolve) => {
            ChromeUtils.compileScript(
                `data:,"use strict";import("chrome://userscripts/content/${this.filename}").catch(console.error)`
            )
                .then((script) => {
                ...
                .catch((ex) =>
                    resolve(ScriptData.onCompileRejection(ex, this.filename))
                )
                .finally(() => {
                    this.#preCompiling = null;
                });
        });
        return this.#preCompiling;
    }
    static onCompileRejection(ex, script) {
        ...
        console.error(
            `@ ${script.filename}: script couldn't be compiled because:`,
            ex
        );
        return null;
    }
    tryLoadIntoWindow(win) {
        ...
        injection.catch((ex) => {
            console.error(
                new Error(`@ ${this.filename}:${ex.lineNumber}`, { cause: ex })
            );
        });
    }
    ...
    getInfo(isEnabledOverride) {
        return ScriptInfo.fromScript(
            this,
            isEnabledOverride === undefined ? this.isEnabled : isEnabledOverride
        );
    }
    registerManifest() {
        if (this.isRunning) {
        ...
        let cmanifest = FS.getEntry(`${this.manifest}.manifest`, {
            baseDirectory: FS.SCRIPT_DIR,
        });
        if (cmanifest.isFile()) {
            Components.manager
            ...
            console.warn(
                `Script '${this.filename}' tried to register a manifest but requested file '${this.manifest}' doesn't exist`
            );
        }
    }
        ...
        [
            {
                ...
                callback: (notification) => {
                    Services.prefs.setBoolPref(
                        "userChromeJS.gBrowser_hack.required",
                        true
                    );
                    utils.restart(false);
                    return false;
...
class UserChrome_js {
    ...
    init() {
        ...
        // gBrowserHack setup
        const gBrowserHackRequired = Services.prefs.getBoolPref(
            "userChromeJS.gBrowser_hack.required",
            false
        )
            ? 2
            : 0;
        const gBrowserHackEnabled = Services.prefs.getBoolPref(
            PREF_GBROWSERHACKENABLED,
            false
        )
            ? 1
            : 0;
        ...
        for (let entry of FS.getEntry("", { baseDirectory: FS.SCRIPT_DIR })) {
            if (
                /^\w+.*(\.uc\.js|\.uc\.mjs|\.sys\.mjs)$/i.test(entry.leafName)
            ) {
                ...
                if (script.manifest) {
                    try {
                        ...
                        console.error(
                            new Error(`@ ${script.filename}`, { cause: ex })
                        );
                    }
                }
    ...
    `);
      ...
      `)
        ...
        menu.parentNode
            .querySelector("#menuUserScriptsPopup")
            .addEventListener("popupshown", updateMenuStatus);
        document.l10n
            .formatValues([
    ...
    handleEvent(aEvent) {
        switch (aEvent.type) {
            ...
            default:
                console.warn(
                    new Error("unexpected event received", { cause: aEvent })
                );
        }
    }
}
