...
class UndoListInTabmenu {
    ...
    get strings() {
        return (
            this._strings ||
            (this._strings =
                RecentlyClosedTabsAndWindowsMenuUtils.strings ||
    ...
    async attachSidebarListener() {
        let TST = await AddonManager.getAddonByID(
            "treestyletab@piro.sakura.ne.jp"
        );
        if (TST)
            SidebarUI._switcherTarget.addEventListener("SidebarShown", this);
    }
    ...
    makePopups(context) {
        ...
        context.addEventListener("popupshowing", (e) => {
            ...
            if (e.target.id === "toolbar-context-menu") {
                ...
                if (
                    toolbarItem &&
                    toolbarItem.localName == "toolbarpaletteitem"
                ) {
    ...
    populateSubmenu(popup, type) {
        ...
        fragment.lastChild.accessKey =
            fragment.lastChild.label.substr(0, 1) || "R";
    ...
    modMethods() {
        RecentlyClosedTabsAndWindowsMenuUtils.navigatorBundle =
        ...
        RecentlyClosedTabsAndWindowsMenuUtils.createEntry = function (
            ...
            if (aClosedTab.image)
                RecentlyClosedTabsAndWindowsMenuUtils.setImage(
                    aClosedTab,
                    element
                );
            ...
            if (!forContext && aTagName != "menuitem") {
                ...
                if (
                    UndoListInTabmenu.config["Enable context menus in panels"]
                ) {
        ...
        RecentlyClosedTabsAndWindowsMenuUtils.createRestoreAllEntry = function (
            ...
            if (aPrefixRestoreAll) {
                aFragment.insertBefore(
                    restoreAllElements,
                    aFragment.firstChild
                );
            } else {
                aFragment.appendChild(
                    aDocument.createXULElement("menuseparator")
                );
...
class RecentlyClosedPanelContext {
    ...
    bookmarkFromTab() {
        ...
        PlacesUIUtils.showBookmarkPagesDialog(
            [
                {
                    uri: Services.io.newURI(activeEntry.url),
