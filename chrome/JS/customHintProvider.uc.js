...
// up a global object (on the chrome window) for showing confirmation hints with
// custom messages. The built-in confirmation hint component can only show a few
// messages built into the browser's localization system. It only accepts l10n
// IDs, so if your script wants to show a custom message with some specific
// string, it won't work. This works just like the built-in confirmation hint,
// and uses the built-in confirmation hint element, but it accepts an arbitrary
// string as a parameter. So you can open a confirmation hint with *any*
// message, e.g. CustomHint.show(anchorNode, "This is my custom message", {hideArrow: true, hideCheck: true, description: "Awesome.", duration: 3000})
