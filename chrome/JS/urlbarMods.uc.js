...
class UrlbarMods {
    ...
    constructor() {
        if (
            UrlbarMods.config["add new tooltips and classes for identity icon"]
        ) {
    ...
    async extendIdentityIcons() {
        ...
        gIdentityHandler._refreshIdentityIcons = function () {
            ...
            if (this._isSecureInternalUI) {
                ...
                icon_label = gNavigatorBundle.getString(
    ...
    styleIdentityIconDragBox() {
        ...
        gIdentityHandler.onDragStart = function (event) {
            ...
            roundRect(
                ...
                backgroundColor,
                varToHex(
                    "var(--tooltip-border-color, var(--arrowpanel-border-color))"
                )
            );
    ...
    urlbarResultsDetailedIcons() {
        ...
        function makeTTSOnboardingResult(
            ...
            let [url] = UrlbarUtils.stripPrefixAndTrim(
                engine.getResultDomain(),
                {
                    stripWww: true
                }
            );
            url = url.substr(
                0,
                url.length - engine.searchUrlPublicSuffix.length
            );
            ...
            eval(
                `TabToSearch.startQuery = async function uc_startQuery` +
                    src3
                        ...
                        .replace(
                            /makeOnboardingResult/g,
                            "makeTTSOnboardingResult"
                        )
            );
        }
...
.urlbarView-row[type="remotetab"][clientType="vr"] {
  --device-icon: url("chrome://browser/skin/device-vr.svg");
    ...
    urlbarResultsSorting() {
        ...
        if (
            !sortSrc.includes(`UrlbarPrefs.get("showSearchSuggestionsFirst")`)
        ) {
            const lazy = {};
            ChromeUtils.defineESModuleGetters(lazy, {
                UrlbarPrefs: "resource:///modules/UrlbarPrefs.sys.mjs",
