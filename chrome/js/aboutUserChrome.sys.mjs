...
AboutUserChrome.prototype = {
    ...
    newChannel(_uri, loadInfo) {
        const ch = Services.io.newChannelFromURIWithLoadInfo(
            this.uri,
            loadInfo
        );
...
function initUserChromeNotifications() {
    ...
    class UserChromeWindowNotifications {
        ...
        #init() {
            ...
            if (!gUserChromeNotifications) {
                gUserChromeNotifications = new UserChromeNotifications(
                    this.utils
                );
            }
        ...
        #updateBanner() {
            ...
            if (notificationId) {
                if (!this.#banner) {
                    ...
                    let banner =
                        sibling.ownerDocument.createXULElement("toolbarbutton");
    ...
    class UserChromeNotifications {
        ...
        get notificationId() {
            ...
            let updates = handles.filter((handle) => {
                ...
                return (
                    Services.vc.compare(newVersion, handle.currentVersion) > 0
                );
            });
            if (!updates.length) {
                return null;
    ...
    class UserChromeManagerEntryPoints {
        ...
        #addMenuitem() {
            let addonsItem =
                this.win.document.getElementById("menu_openAddons");
    ...
    lazy.EveryWindow.registerCallback(
        ...
        (win) => {
            win.userChromeWindowNotifications =
                new UserChromeWindowNotifications(win);
