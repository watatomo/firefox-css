...
class DebugExtension {
    ...
    handleEvent(e) {
        ...
        if (popup.className.includes("Submenu-Popup")) {
            actions.forEach((url, type) => {
                popup.querySelector(`.customize-context-${type}`).disabled =
                    !url;
            });
        } else {
            popup.querySelector(
                ".customize-context-ViewDocs-Submenu"
            ).disabled = [...actions.values()].every((url) => !url);
    ...
    makeMainMenu(popup) {
        ...
        this.maybeSetAttributes(menu, {
            ...
            contexttype:
                popup === this.pageActionContext ? void 0 : "toolbaritem"
        });
        popup
            .querySelector(
