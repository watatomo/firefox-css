...
class ExtensionOptionsWidget {
    ...
    static config = {
        ...
        "Addon ID blacklist": JSON.parse(
            Services.prefs.getCharPref(
                "extensionOptionsPanel.addonIDBlacklist",
                "[]"
            )
        ),
    ...
    constructor() {
        ...
        if (
            /^chrome:\/\/browser\/content\/browser.(xul||xhtml)$/i.test(
                location
            ) &&
            ...
            CustomizableUI.createWidget({
                ...
                onBeforeCreated: (aDoc) => {
                    ...
                    if (eop.config["Show header"]) {
                        let header = view.appendChild(
                            eop.create(aDoc, "vbox", {
                                id: "eom-mainView-panel-header"
                            })
                        );
                        let heading = header.appendChild(
                            eop.create(aDoc, "label")
                        );
                        ...
                        view.appendChild(
                            aDoc.createXULElement("toolbarseparator")
                        );
                    }
                    ...
                    eop.fluentSetup(aDoc).then(() =>
                        eop.swapAddonsButton(aDoc)
                    );
                },
    ...
    swapAddonsButton(aDoc) {
        ...
        this.setAttributes(
            win.PanelUI.mainView.querySelector(
    ...
    populatePanelBody(e, addons) {
        ...
        if (
            ...
            [...appMenuMultiView._viewStack.children].forEach((panel) => {
                if (
                    panel.id !== view.id &&
                    panel.id.includes("PanelUI-eom-addon-")
                ) {
                    panel.remove();
                }
            });
            ...
            .forEach((addon) => {
                ...
                if (
                    ...
                    if (
                        ...
                        disabledSubheader.textContent =
                            this.aboutAddonsStrings.disabled;
                    }
                    ...
                    if (!addon.isActive)
                        subviewbutton.classList.add("disabled");
            ...
            if (
                ...
                subviewbutton.addEventListener("command", async (e) => {
                    ...
                    subviewbutton.parentElement
                        ...
                        .forEach((btn) => {
                            btn.classList[
                                btn._Addon?.isActive ? "remove" : "add"
                            ]("disabled");
        ...
        view.appendChild(
            this.create(doc, "toolbarbutton", {
                ...
                key: "key_openAddons",
                shortcut: win.ShortcutUtils.prettifyShortcut(
                    win.key_openAddons
                ),
    ...
    async setAddonMessage(doc, subviewbutton, addon) {
        ...
        const formatString = (type, args) => {
            return new Promise((resolve) => {
                doc.l10n
                    .formatMessages([
                        { id: `details-notification-${type}`, args }
                    ])
        ...
        if (message) {
            ...
            if (message.checked)
                subviewbutton.setAttribute("enable-checked", true);
            if (message.label) {
    ...
    buildSubView(subviewbutton, addon) {
        ...
        viewSrcButton.addEventListener("command", () =>
            this.openArchive(addon)
        );
        ...
        homePageButton.addEventListener("command", () => {
            win.switchToTabHavingURI(
                addon.homepageURL || addon.supportURL,
                true,
                {
                    inBackground: false,
                    triggeringPrincipal:
        ...
        copyIdButton.addEventListener("command", () => {
            ...
            let PMV =
                view.panelMultiView &&
                win.PanelMultiView.forNode(view.panelMultiView);
            if (PMV) {
                let panel = PMV._panel;
                if (
                    panel &&
                    PMV._getBoundsWithoutFlushing(panel.anchorNode)?.width
                ) {
        ...
        view.addEventListener("ViewShowing", () => {
            ...
            updates.hidden = !(
                addon.permissions & win.AddonManager.PERM_CAN_UPGRADE
            );
            ...
            setPrivateState(addon, privateGroup);
            shortcutsButton.hidden = !this.extensionForAddonId(addon.id)
                ?.shortcuts?.manifestCommands?.size;
            disableButton.setAttribute(
                "label",
                addon.userDisabled
                    ? l10n["Enable addon label"]
    ...
    openAddonOptions(addon, win) {
        ...
        switch (Number(addon.optionsType)) {
            case 5:
                win.BrowserOpenAddonsMgr(
                    `addons://detail/${win.encodeURIComponent(
                        addon.id
                    )}/preferences`
                );
                break;
            ...
            case 1:
                ...
                if (
                    win.Services.prefs.getBoolPref(
