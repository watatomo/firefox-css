...
// @description
/*
By default, Firefox hides the tracking protection while the current tab is open to the default new tab page, and while the user is typing into the url bar. Hiding the icon while the user is typing is unnecessary, since although `pageproxystate` has changed, the content principal is still the same and clicking the tracking protection icon to open the popup still works. Opening the popup while `pageproxystate` is invalid still loads the tracking details and options for the current content URI. But hiding the icon on the new tab page is necessary, because the tracking protection icon is hidden on about:blank.

If you use an extension to set a custom new tab page, you will see the tracking protection icon briefly disappear when opening a new tab, before reappearing as the custom new tab page loads. That is because about:blank loads before the custom new tab page loads. So the icon is hidden and unhidden in the span of a hundred milliseconds or so. This looks very ugly, so my stylesheet has always prevented the tracking protection icon from being hidden on any page, including about:blank. That way at least it doesn't disappear.

But CSS isn't a great solution, because there are a number of pages for which the tracking protection icon does nothing. The protection handler can't handle internal pages, for example. Previously I just disabled pointer events on the icon when it was supposed to be hidden. But I think this script is a better solution. If this script is not installed, [duskFox][] will default to those older methods I just mentioned.

But if the script is installed, it will restore the built-in behavior of hiding the tracking protection icon on internal pages, only it will also hide the icon on the user's custom new tab page. The icon will still be visible if you're on a valid webpage (anything but about, chrome, and resource URIs), even if you begin typing in the urlbar.

[duskFox]: https://github.com/aminomancer/uc.css.js
*/
// @downloadURL    https://cdn.jsdelivr.net/gh/aminomancer/uc.css.js@master/JS/hideTrackingProtectionIconOnCustomNewTabPage.uc.js
// @updateURL      https://cdn.jsdelivr.net/gh/aminomancer/uc.css.js@master/JS/hideTrackingProtectionIconOnCustomNewTabPage.uc.js
        ...
        _getAnchor(panel) {
            if (this.hideBookmarks) {
                for (let id of [
                    ...
                    if (node && !node.hidden) {
                        let bounds =
                            window.windowUtils.getBoundsWithoutFlushing(node);
        ...
        constructor() {
            ...
            gProtectionsHandler.onLocationChange = function onLocationChange() {
                ...
                if (currentURL.startsWith("view-source:")) {
                    ...
                    this._trackingProtectionIconContainer.removeAttribute(
                        "view-source"
                    );
                }
                ...
                if (
                    !lazy.ContentBlockingAllowList.canHandle(
